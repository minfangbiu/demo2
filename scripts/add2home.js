...
var nav = navigator,
	...
	options = {
		...
		bottomOffset: 20,			// Distance of the balloon from bottom
	...
	intl = {
		...
		en_us: 'Install this web app on your %device: tap this %icon icon and then <strong>Add to Home Screen</strong>.',
...
function ready () {
	...
	div.innerHTML = touchIcon + options.message.replace('%device', platform).replace('%icon', OSVersion >= 4.2 ? '<span class="share"></span>' : '<span class="plus">+</span>') + (options.arrow ? '<span class="arrow"></span>' : '') + '<span class="close"><p class="close-design">\u00D7</p></span>';
	...
	switch (options.animationOut) {
		case 'drop':
			if (isIPad) {
				...
				duration = '0.6s';
				posY = posY + el.offsetHeight + options.bottomOffset;
			}
			break;
		case 'bubble':
			if (isIPad) {
